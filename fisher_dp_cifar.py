...
def load_and_test_models(device, test_loader, critical_loader):
    """加载并测试已保存的模型"""
    models = {}
    results = {}
    ...
    for name, file_name in model_files.items():
        ...
        model.eval()
        models[name] = model
        results[name] = checkpoint['accuracy']
    ...
    # 计算效用下降
    if len(models) == 3:  # 确保所有三个模型都存在
        utility_drop_results = compute_utility_drop(
            models["基线模型"],
            models["DP模型"],
            models["校准DP模型"],
            critical_loader,
...
def main():
    ...
    # 尝试加载已保存的模型
    print("检查是否存在已保存的模型...")
    models, results = load_and_test_models(device, test_loader, critical_loader)
